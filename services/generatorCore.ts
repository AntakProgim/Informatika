...
import { GeneratorParams, LessonPlan } from "../types";

export const differentiatedTasksSchema: Schema = {
  type: Type.OBJECT,
  properties: {
    level1: { 
      type: Type.ARRAY, 
...
export function buildLessonPrompt(params: GeneratorParams): string {
  let promptText = `
...
export async function executeLessonPlanGeneration(
  ...
  const promptText = buildLessonPrompt(params);
  ...
  if (!text) {
    throw new Error("Gautas tuščias atsakymas iš DI modelio.");
  }

  const plan: LessonPlan = JSON.parse(text);

  plan.teacherResources = [];
  if (params.additionalLinks) {
    plan.teacherResources.push(...params.additionalLinks.map((l: string) => ({ name: l, type: "link" as const, url: l })));
  }
  if (params.uploadedFiles) {
    plan.teacherResources.push(...params.uploadedFiles.map((f: { name: string }) => ({ name: f.name, type: "file" as const })));
  }
